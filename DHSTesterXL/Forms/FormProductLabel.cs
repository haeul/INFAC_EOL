...
namespace DHSTesterXL
{
    ...
    public partial class FormProduct : Form
    {
        ...
        // 로고 기본 폴더
        private const string DefaultLogoDirectory = @"D:\INFAC_20250915\DHS_EOL_V3\DHSTesterXL\Images";
        private string _lastLogoDir = null;

        // ───────────────────── Label Grid ─────────────────────
        private const string ColumnNameSequence = "Seq";
        private const string ColumnNameField = "Field";
        private const string ColumnNameData = "Data";
        private const string ColumnNamePositionXMm = "Xmm";
        private const string ColumnNamePositionYMm = "Ymm";
        private const string ColumnNameSizeMm = "Fontmm";
        private const string ColumnNameScaleX = "Xscale";   // 가로 비율
        private const string ColumnNameScaleY = "Yscale";   // 세로 비율

        // 표시 체크박스(미리보기/인쇄)
        private const string ColumnNamePreviewEnabled = "미리보기";
        private const string ColumnNamePrintEnabled = "인쇄";
        ...
        public void LoadFromProduct(ProductConfig cfg)
        {
            ...
            try
            {
                ...
                // 라벨 크기 UI 기본 세팅
                if (numLabelWidth != null) numLabelWidth.Value = (decimal)(_style.LabelWidthMm > 0 ? _style.LabelWidthMm : 60.0);
                if (numLabelHeight != null) numLabelHeight.Value = (decimal)(_style.LabelHeightMm > 0 ? _style.LabelHeightMm : 15.0);
        ...
        public string BuildZpl() { GetGridLabelValue(); return BuildZplFromUi(DefaultDpi); }
        ...
        public bool PrintTo(string printerName)
        {
            ...
            GetGridLabelValue();
            string zpl = BuildZplFromUi(DefaultDpi);
        ...
        partial void Label_Init()
        {
            ...
            if (string.IsNullOrWhiteSpace(_lastLogoDir))
            {
                ...
                else if (Directory.Exists(DefaultLogoDirectory))
                    _lastLogoDir = DefaultLogoDirectory;
                else
        ...
        */
        ...
        private void btnPrintFontTest_Click(object sender, EventArgs e)
        {
            ...
            PrintAllTtfSamples(printer, DefaultDpi);
        }
