...
namespace DHSTesterXL
{
    public partial class PNFCTouchFD : IDHSModel
    {
        ...
        private void NFCTouchTestStep_SerialNumWriteSend(int channel)
        {
            ...
            _nextSerialNo = GSystem.ProductSettings.ProductInfo.TypeName.Substring(GSystem.ProductSettings.ProductInfo.TypeName.Length - 3) + GSystem.ProductSettings.ProductInfo.PartNo.Substring(GSystem.ProductSettings.ProductInfo.PartNo.Length - 5) + "LOT1" + sn.ToString("D04") + "0000";
        ...
        private void NFCTouchTestStep_HighPowerOnWait(int channel, uint rxCanID, ref XLcanRxEvent receivedEvent)
        {
            // 완료 대기
            if (!GSystem.DedicatedCTRL.GetCommandActiveCurrent(channel) || !GSystem.DedicatedCTRL.GetCompleteActiveCurrent(channel))
                return;
        ...
        private void NFCTouchTestStep_HighPowerWakeUpWait(int channel, uint rxCanID, ref XLcanRxEvent receivedEvent)
        {
            //
            // TODO: WakeUp 타임아웃 처리
            //
