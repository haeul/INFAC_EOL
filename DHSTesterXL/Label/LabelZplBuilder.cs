...
namespace DHSTesterXL
{
    public partial class FormProduct
    {
        ...
        {
            int printWidthDots = MmToDots(_style.LabelWidthMm, dpi); // Print Width
            int labelLengthDots = MmToDots(_style.LabelHeightMm, dpi); // Label Length

            // 텍스트 페이로드
            string brand = _style.BrandText ?? "";
            string part = _style.PartText ?? "";
            string hw = GetGridText(RowKey.HW, _style.HardwareText ?? "");
            string sw = GetGridText(RowKey.SW, _style.SoftwareText ?? "");
            ...
            // ───────────────────── 로고 (^GFA, 좌상단 기준) ─────────────────────
            if (_style.IsLogoPrintEnabled && !string.IsNullOrWhiteSpace(_style.LogoImagePath) && File.Exists(ResolveLogoPath(_style.LogoImagePath)))
            {
                if (_logoBitmap == null) LoadLogoBitmap();
                if (_logoBitmap != null)
                {
                    var logoRow = GetRow(RowKey.Logo);
                    double logoScaleX = ReadScaleCell(logoRow, ColumnNameScaleX, 1.0);
                    double logoScaleY = ReadScaleCell(logoRow, ColumnNameScaleY, 1.0);

                    double logoAspectRatio = _logoBitmap.Width / (double)_logoBitmap.Height;
                    int logoHeightDots = Math.Max(1, MmToDots(_style.LogoHeightMm * logoScaleY, dpi));
                    int logoWidthDots = Math.Max(1, MmToDots(_style.LogoHeightMm * logoAspectRatio * logoScaleX, dpi));
                    ...
                    int logoOriginXDots = MmToDots(_style.LogoXMm, dpi);
                    int logoOriginYDots = MmToDots(_style.LogoYMm, dpi);
                    zplBuilder.AppendLine($"^FO{logoOriginXDots},{logoOriginYDots}{logoGraphicFieldData}^FS");
                }
            }

            // ───────────────────── Brand (텍스트, 좌상단 기준) ─────────────────────
            if (_style.IsBrandPrintEnabled && !string.IsNullOrEmpty(brand))
            {
                ...
                int fontHeightDots = MmToDots(_style.BrandFontMm * scaleY, dpi);
                int fontWidthDots = (int)Math.Round(fontHeightDots * scaleX);

                int originXDots = MmToDots(_style.BrandXMm, dpi);
                int originYDots = MmToDots(_style.BrandYMm, dpi);
            ...
            // ───────────────────── Part ─────────────────────
            if (_style.IsPartPrintEnabled && !string.IsNullOrEmpty(part))
            {
                ...
                int fontHeightDots = MmToDots(_style.PartFontMm * scaleY, dpi);
                int fontWidthDots = (int)Math.Round(fontHeightDots * scaleX);

                if (_style.PartXMm <= 0)
                {
                    int partY = MmToDots(_style.PartYMm, dpi);
                ...
                {
                    int originXDots = MmToDots(_style.PartXMm, dpi);
                    int originYDots = MmToDots(_style.PartYMm, dpi);
            ...
            // ───────────────────── HW ─────────────────────
            if (_style.IsHardwarePrintEnabled && !string.IsNullOrEmpty(hw))
            {
                ...
                int fontHeightDots = MmToDots(PositiveOr(_style.HardwareFontMm, 2.6) * scaleY, dpi);
                int fontWidthDots = (int)Math.Round(fontHeightDots * scaleX);

                int originXDots = MmToDots(_style.HardwareXMm, dpi);
                int originYDots = MmToDots(_style.HardwareYMm, dpi);
            ...
            // ───────────────────── SW ─────────────────────
            if (_style.IsSoftwarePrintEnabled && !string.IsNullOrEmpty(sw))
            {
                ...
                int fontHeightDots = MmToDots(PositiveOr(_style.SoftwareFontMm, 2.6) * scaleY, dpi);
                int fontWidthDots = (int)Math.Round(fontHeightDots * scaleX);

                int originXDots = MmToDots(_style.SoftwareXMm, dpi);
                int originYDots = MmToDots(_style.SoftwareYMm, dpi);
            ...
            // LOT
            if (_style.IsLotPrintEnabled && !string.IsNullOrEmpty(lot))
            {
                ...
                int fontHeightDots = MmToDots(PositiveOr(_style.LotFontMm, 2.6) * scaleY, dpi);
                int fontWidthDots = (int)Math.Round(fontHeightDots * scaleX);

                int originXDots = MmToDots(_style.LotXMm, dpi);
                int originYDots = MmToDots(_style.LotYMm, dpi);
            ...
            // SN
            if (_style.IsSerialPrintEnabled && !string.IsNullOrEmpty(sn))
            {
                ...
                int fontHeightDots = MmToDots(PositiveOr(_style.SerialFontMm, 2.6) * scaleY, dpi);
                int fontWidthDots = (int)Math.Round(fontHeightDots * scaleX);

                int originXDots = MmToDots(_style.SerialXMm, dpi);
                int originYDots = MmToDots(_style.SerialYMm, dpi);
            ...
            if (_style.IsPbPrintEnabled)
            {
                ...
                int badgeWidthDots = Math.Max(1, MmToDots(_style.BadgeDiameterMm * scaleX, dpi));
                int badgeHeightDots = Math.Max(1, MmToDots(_style.BadgeDiameterMm * scaleY, dpi));
                int originXDots = MmToDots(_style.BadgeXMm, dpi);
                int originYDots = MmToDots(_style.BadgeYMm, dpi);
            ...
            // ───────────────────── Data Matrix (좌상단 기준) ─────────────────────
            if (_style.IsDataMatrixPrintEnabled)
            {
                // 모듈(셀) 1칸의 mm → 정수 도트
                int moduleDotsPerCell = Math.Max(1, MmToDots(Math.Max(0.1, _style.DataMatrixModuleSizeMm), dpi));

                // 좌표
                int originXDots = MmToDots(_style.DataMatrixXMm, dpi);
                int originYDots = MmToDots(_style.DataMatrixYMm, dpi);
            ...
            // Rating
            if (_style.IsRatingPrintEnabled)
            {
                int originXDots = MmToDots(_style.RatingXMm, dpi);
                int originYDots = MmToDots(_style.RatingYMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.RatingFontMm, 2.6), dpi);
            ...
            // FCC ID
            if (_style.IsFccIdPrintEnabled)
            {
                int originXDots = MmToDots(_style.FccIdXMm, dpi);
                int originYDots = MmToDots(_style.FccIdYMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.FccIdFontMm, 2.6), dpi);
            ...
            // IC ID
            if (_style.IsIcIdPrintEnabled)
            {
                int originXDots = MmToDots(_style.IcIdXMm, dpi);
                int originYDots = MmToDots(_style.IcIdYMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.IcIdFontMm, 2.6), dpi);
            ...
            // Item1
            if (_style.IsItem1PrintEnabled)
            {
                int originXDots = MmToDots(_style.Item1XMm, dpi);
                int originYDots = MmToDots(_style.Item1YMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.Item1FontMm, 2.6), dpi);
            ...
            // Item2
            if (_style.IsItem2PrintEnabled)
            {
                int originXDots = MmToDots(_style.Item2XMm, dpi);
                int originYDots = MmToDots(_style.Item2YMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.Item2FontMm, 2.6), dpi);
            ...
            // Item3
            if (_style.IsItem3PrintEnabled)
            {
                int originXDots = MmToDots(_style.Item3XMm, dpi);
                int originYDots = MmToDots(_style.Item3YMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.Item3FontMm, 2.6), dpi);
            ...
            // Item4
            if (_style.IsItem4PrintEnabled)
            {
                int originXDots = MmToDots(_style.Item4XMm, dpi);
                int originYDots = MmToDots(_style.Item4YMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.Item4FontMm, 2.6), dpi);
            ...
            // Item5
            if (_style.IsItem5PrintEnabled)
            {
                int originXDots = MmToDots(_style.Item5XMm, dpi);
                int originYDots = MmToDots(_style.Item5YMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(_style.Item5FontMm, 2.6), dpi);
        ...
        private string BuildEtcsQrPayloadFromUi()
        {
            ...
            payloadBuilder.Append(RS).Append("06");        // 버전

            payloadBuilder.Append(GS).Append("V").Append(vendorValue);
            payloadBuilder.Append(GS).Append("P").Append(partNumberValue);

            // S/E는 비어도 칸(=GS) 자체는 유지 → 필드 밀림 방지
            payloadBuilder.Append(GS);
            ...
            payloadBuilder.Append(GS).Append("T").Append(traceValue).Append(specialValue).Append(initialValue).Append(companyAreaValue);

            // 트레일러
            payloadBuilder.Append(GS).Append(RS).Append(EOT);
    ...
    public static class ZebraZplFacade
    {
        ...
        private static string BuildEtcsDm(EtcsSettings d)
        {
            ...
            zplBuilder.Append(RS).Append("06"); // 버전

            zplBuilder.Append(GS).Append("V").Append(v);
            zplBuilder.Append(GS).Append("P").Append(p);

            // S/E는 비어도 GS 토큰은 남겨 필드 밀림 방지
            zplBuilder.Append(GS);
            //if (!string.IsNullOrEmpty(s))
            ...
            zplBuilder.Append(GS).Append("T").Append(t).Append(a1).Append(m).Append(c);

            zplBuilder.Append(GS).Append(RS).Append(EOT);
            return zplBuilder.ToString();
        }
        ...
        )
        {
            ...
            int printWidthDots = MmToDots(labelStyle.LabelWidthMm, dpi);
            int labelLengthDots = MmToDots(labelStyle.LabelHeightMm, dpi);
            ...
            if (labelStyle.IsLogoPrintEnabled && !string.IsNullOrWhiteSpace(labelStyle.LogoImagePath))
            {
                try
                {
                    using (var sourceLogoBitmap = loadLogoBitmap?.Invoke())
                    {
                        if (sourceLogoBitmap != null)
                        {
                            ...
                            int logoHeightDots = Math.Max(1, MmToDots(labelStyle.LogoHeightMm, dpi));
                            ...
                            using (var graphics = Graphics.FromImage(canvas))
                            {
                                ...
                                int logoOriginXDots = MmToDots(labelStyle.LogoXMm, dpi);
                                int logoOriginYDots = MmToDots(labelStyle.LogoYMm, dpi);
                                zplBuilder.AppendLine($"^FO{logoOriginXDots},{logoOriginYDots}{logoGraphicFieldData}^FS");
                            }
                        }
                    }
            ...
            // ── Brand
            if (labelStyle.IsBrandPrintEnabled && !string.IsNullOrEmpty(payload?.Company))
            {
                int fontHeightDots = MmToDots(PositiveOr(labelStyle.BrandFontMm, 2.6), dpi);
                int fontWidthDots = fontHeightDots; // ScaleX 없음 → 폭=높이로 처리
                int originXDots = MmToDots(labelStyle.BrandXMm, dpi);
                int originYDots = MmToDots(labelStyle.BrandYMm, dpi);
            ...
            if (labelStyle.IsPartPrintEnabled && !string.IsNullOrEmpty(payload?.PartNo))
            {
                int fontHeightDots = MmToDots(PositiveOr(labelStyle.PartFontMm, 2.6), dpi);
                int fontWidthDots = fontHeightDots;
                if (labelStyle.PartXMm <= 0)
                {
                    int originYDots = MmToDots(labelStyle.PartYMm, dpi);
                ...
                {
                    int originXDots = MmToDots(labelStyle.PartXMm, dpi);
                    int originYDots = MmToDots(labelStyle.PartYMm, dpi);
            ...
            PrintText(zplBuilder, labelStyle.IsHardwarePrintEnabled, labelStyle.HardwareXMm, labelStyle.HardwareYMm, labelStyle.HardwareFontMm, payload?.HW, dpi);
            PrintText(zplBuilder, labelStyle.IsSoftwarePrintEnabled, labelStyle.SoftwareXMm, labelStyle.SoftwareYMm, labelStyle.SoftwareFontMm, payload?.SW, dpi);
            PrintText(zplBuilder, labelStyle.IsLotPrintEnabled, labelStyle.LotXMm, labelStyle.LotYMm, labelStyle.LotFontMm, payload?.LOT, dpi);
            PrintText(zplBuilder, labelStyle.IsSerialPrintEnabled, labelStyle.SerialXMm, labelStyle.SerialYMm, labelStyle.SerialFontMm, payload?.SN, dpi);

            // ── Pb 배지 : 지름만 존재 → 타원 크기 w=h로
            if (labelStyle.IsPbPrintEnabled)
            {
                int badgeDiameterDots = Math.Max(1, MmToDots(labelStyle.BadgeDiameterMm, dpi));
                int originXDots = MmToDots(labelStyle.BadgeXMm, dpi);
                int originYDots = MmToDots(labelStyle.BadgeYMm, dpi);
                int strokeThickness = 2;
            ...
            if (labelStyle.IsDataMatrixPrintEnabled && !string.IsNullOrWhiteSpace(dataMatrixPayload))
            {
                int moduleDotsPerCell = Math.Max(1, MmToDots(Math.Max(0.1, labelStyle.DataMatrixModuleSizeMm), dpi));
                int originXDots = MmToDots(labelStyle.DataMatrixXMm, dpi);
                int originYDots = MmToDots(labelStyle.DataMatrixYMm, dpi);
            ...
            // ── Rating / FCC / IC
            if (labelStyle.IsRatingPrintEnabled && !string.IsNullOrEmpty(labelStyle.RatingText))
            {
                int originXDots = MmToDots(labelStyle.RatingXMm, dpi);
                int originYDots = MmToDots(labelStyle.RatingYMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(labelStyle.RatingFontMm, 2.6), dpi);
                zplBuilder.AppendLine($"^FO{originXDots},{originYDots}^A0N,{fontHeightDots},{fontHeightDots}^FD{Escape(labelStyle.RatingText)}^FS");
            }
            if (labelStyle.IsFccIdPrintEnabled && !string.IsNullOrEmpty(payload?.FCCID ?? labelStyle.FccIdText))
            {
                int originXDots = MmToDots(labelStyle.FccIdXMm, dpi);
                int originYDots = MmToDots(labelStyle.FccIdYMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(labelStyle.FccIdFontMm, 2.6), dpi);
            ...
            if (labelStyle.IsIcIdPrintEnabled && !string.IsNullOrEmpty(payload?.ICID ?? labelStyle.IcIdText))
            {
                int originXDots = MmToDots(labelStyle.IcIdXMm, dpi);
                int originYDots = MmToDots(labelStyle.IcIdYMm, dpi);
                int fontHeightDots = MmToDots(PositiveOr(labelStyle.IcIdFontMm, 2.6), dpi);
