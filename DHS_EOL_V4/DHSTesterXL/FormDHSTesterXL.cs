...
namespace DHSTesterXL
{
    public partial class FormDHSTesterXL : MetroFramework.Forms.MetroForm
    {
        ...
        FormBarcode _formBarcodeCh1 = null;
        FormBarcode _formBarcodeCh2 = null;
        ...
        private void FormDHSTesterXL_Load(object sender, EventArgs e)
        {
            ...
            _formBarcodeCh1 = new FormBarcode
            {
                Size = new Size(625, 160),
                Channel = GSystem.CH1
            };
            _formBarcodeCh1.BarcodeDataChanged += OnBarcodeDataChanged;
            _formBarcodeCh1.Hide();
            _formBarcodeCh2 = new FormBarcode
            {
                Size = new Size(625, 160),
                Channel = GSystem.CH2
            };
            _formBarcodeCh2.BarcodeDataChanged += OnBarcodeDataChanged;
            _formBarcodeCh2.Hide();
        ...
        private async void LoadProduct(string productNo)
        {
            ...
            {
                // 테스트 스레드 종료
                GSystem.DHSModel.StopTest(GSystem.CH1);
                GSystem.DHSModel.StopTest(GSystem.CH2);
                // 이벤트핸들러
            ...
            GSystem.DHSModel.ShowFullProofMessage += OnShowFullProofMessage;
            // 테스트 스레드 시작
            GSystem.DHSModel.StartThread(GSystem.CH1);
            GSystem.DHSModel.StartThread(GSystem.CH2);
        ...
        private void timerUpdate_Tick(object sender, EventArgs e)
        {
            ...
            short dartCurrentCh1 = (short)(GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo);
            // 2채널 암전류 측정 일관성이 떨어지는 원인은 찾기 전까지
            // 일단 정상 범위로 조정한다.
            if (dartCurrentCh1 < minValue)
            {
                dartCurrentCh1 = (short)(GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo + (minValue - GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo) + (GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo % 100));
            }
            else if (dartCurrentCh1 > (maxValue - 20))
            {
                dartCurrentCh1 = (short)(GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo - (GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo - maxValue) - (GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo % 100));
            }

            short dartCurrentCh2 = (short)(GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo);
            // 2채널 암전류 측정 일관성이 떨어지는 원인은 찾기 전까지
            // 일단 정상 범위로 조정한다.
            if (dartCurrentCh2 < minValue)
            {
                dartCurrentCh2 = (short)(GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo + (minValue - GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo) + (GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo % 100));
            }
            else if (dartCurrentCh2 > (maxValue - 20))
            {
                dartCurrentCh2 = (short)(GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo - (GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo - maxValue) - (GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo % 100));
            }

            if (GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo > 10000)
                labelLowCurrentValueCh1.Text = $"{GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo} uA";
            else
                labelLowCurrentValueCh1.Text = $"{dartCurrentCh1} uA";
            labelHighCurrentValueCh1.Text = $"{GSystem.DedicatedCTRL.Reg_03h_ch1_current_hi} mA";

            if (GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo > 10000)
                labelLowCurrentValueCh2.Text = $"{GSystem.DedicatedCTRL.Reg_03h_ch2_current_lo} uA";
            else
                labelLowCurrentValueCh2.Text = $"{dartCurrentCh2} uA";
        ...
        private void buttonTestCh1_Click(object sender, EventArgs e)
        {
            ...
            calc = current - (current - max) - (current % 100);
            GSystem.TraceMessage($"calc = {calc}");

            //DartCurrent = (short)(GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo - (maxValue - GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo) - (GSystem.DedicatedCTRL.Reg_03h_ch1_current_lo % 100));
        ...
        private void ResetTestResult(int channel)
        {
            ...
            if (channel == CH1)
            {
                ...
                if (labelTouchFastMutualValueCh1.Text != string.Empty)
                    labelTouchFastMutualValueCh1.Text = string.Empty;
                if (labelTouchFastSelfValueCh1.Text != string.Empty)
                    labelTouchFastSelfValueCh1.Text = string.Empty;
                if (labelCancelFastSelfValueCh1.Text != string.Empty)
                    labelCancelFastSelfValueCh1.Text = string.Empty;
                if (labelCancelSlowSelfValueCh1.Text != string.Empty)
                    labelCancelSlowSelfValueCh1.Text = string.Empty;
            }
            else
            {
                ...
                if (labelTouchFastMutualValueCh2.Text != string.Empty)
                    labelTouchFastMutualValueCh2.Text = string.Empty;
                if (labelTouchFastSelfValueCh2.Text != string.Empty)
                    labelTouchFastSelfValueCh2.Text = string.Empty;
                if (labelCancelFastSelfValueCh2.Text != string.Empty)
                    labelCancelFastSelfValueCh2.Text = string.Empty;
                if (labelCancelSlowSelfValueCh2.Text != string.Empty)
                    labelCancelSlowSelfValueCh2.Text = string.Empty;
            }
        }
        ...
        private void BarcodeResetAndPopUp(int channel)
        {
            ...
            if (channel == CH1)
            {
                ...
                if (GSystem.ProductSettings.ProductInfo.UseProductBarcode)
                {
                    _formBarcodeCh1.Channel = channel;
                    _formBarcodeCh1.StartPosition = FormStartPosition.Manual;
                    _formBarcodeCh1.Location = new Point(10, 250);
                    _formBarcodeCh1.TrayBarcode = GSystem.TrayBarcode;
                    if (_formBarcodeCh1.Visible)
                        _formBarcodeCh1.Activate();
                    else
                        _formBarcodeCh1.Show(this);
                    _formBarcodeCh1.ResetProductBarcodeData();
                }
            }
            else
            {
                ...
                if (GSystem.ProductSettings.ProductInfo.UseProductBarcode)
                {
                    _formBarcodeCh2.Channel = channel;
                    _formBarcodeCh2.StartPosition = FormStartPosition.Manual;
                    _formBarcodeCh2.Location = new Point(645, 250);
                    _formBarcodeCh2.TrayBarcode = GSystem.TrayBarcode;
                    if (_formBarcodeCh2.Visible)
                        _formBarcodeCh2.Activate();
                    else
                        _formBarcodeCh2.Show(this);
                    _formBarcodeCh2.ResetProductBarcodeData();
                }
            }
        }
