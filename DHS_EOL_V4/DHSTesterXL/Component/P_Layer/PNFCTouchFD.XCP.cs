...
namespace DHSTesterXL
{
    ...
    public partial class PNFCTouchFD : IDHSModel
    {
        ...
        public async Task<bool> GetTouchAsync(int channel)
        {
            ...
            string capaDataFilePath = $"{GSystem.SystemData.GeneralSettings.DataFolderCap}\\{ProductSettings.ProductInfo.PartNo}\\{DateTime.Now:yyyy}\\{DateTime.Now:MM}\\{DateTime.Now:dd}";
        ...
        {
            if (_tickXcpTouchTimeout[channel].MoreThan(300))
            {
                // retry
                ...
                if (waitResult != WaitResults.WAIT_TIMEOUT)
                {
                    ...
                    while (xlStatus != XL_Status.XL_ERR_QUEUE_IS_EMPTY)
                    {
                        ...
                        if (xlStatus == XL_Status.XL_SUCCESS)
                        {
                            ...
                            if (rxCanID == ProductSettings.XcpResID)
                            {
        ...
        {
            if (_tickXcpCancelTimeout[channel].MoreThan(300))
            {
                // retry
