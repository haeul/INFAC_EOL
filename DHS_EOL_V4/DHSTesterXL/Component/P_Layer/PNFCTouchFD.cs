...
namespace DHSTesterXL
{
    ...
    public partial class PNFCTouchFD : IDHSModel
    {
        ...
        private byte[,] _serialNumberBytes  = new byte[ChannelCount,  20];
        ...
        public Task<XL_Status> OpenPort(int channel)
        {
            ...
            TraceMessage($"OpenPort - NFCTouchFD");

            status = CanXL.Driver.XL_OpenPort(ref _portHandle[channel], CanXL.AppName, CanXL._accessMask[channel], 
                ref CanXL._permissionMask[channel], 16000, XL_InterfaceVersion.XL_INTERFACE_VERSION_V4, XL_BusTypes.XL_BUS_TYPE_CAN);
            ...
            // Set Can FD configuration
            XLClass.XLcanFdConf canFdConf = new XLClass.XLcanFdConf();
            uint arbitrationBitRate = Convert.ToUInt32(ProductSettings.CommSettings.ArbtBitRate);
            uint dataBitRate        = Convert.ToUInt32(ProductSettings.CommSettings.DataBitRate);
            uint tseg1Abr           = Convert.ToUInt32(ProductSettings.CommSettings.ArbtTseg1  );
            uint tseg2Abr           = Convert.ToUInt32(ProductSettings.CommSettings.ArbtTseg2  );
            uint sjwAbr             = Convert.ToUInt32(ProductSettings.CommSettings.ArbtSjw    );
            uint tseg1Dbr           = Convert.ToUInt32(ProductSettings.CommSettings.DataTseg1  );
            uint tseg2Dbr           = Convert.ToUInt32(ProductSettings.CommSettings.DataTseg2  );
            uint sjwDbr             = Convert.ToUInt32(ProductSettings.CommSettings.DataSjw    );

            // arbitration bitrate
            canFdConf.arbitrationBitRate = arbitrationBitRate;
            canFdConf.tseg1Abr = tseg1Abr; // 6;
            canFdConf.tseg2Abr = tseg2Abr; // 3;
            canFdConf.sjwAbr = sjwAbr; // 2;

            // data bitrate
            canFdConf.dataBitRate = dataBitRate;
            canFdConf.tseg1Dbr = tseg1Dbr;// 6;
            canFdConf.tseg2Dbr = tseg2Dbr;// 3;
            canFdConf.sjwDbr = sjwDbr;// 2;

            if (CanXL._canFdModeNoIso > 0)
                canFdConf.options = (byte)XLDefine.XL_CANFD_ConfigOptions.XL_CANFD_CONFOPT_NO_ISO;
            else
                canFdConf.options = 0;

            status = CanXL.Driver.XL_CanFdSetConfiguration(_portHandle[channel], CanXL._accessMask[channel], canFdConf);
            GSystem.Logger.Info ($"Set CAN FD Config     : [CH.{channel + 1}] {status}");
            GSystem.TraceMessage($"Set CAN FD Config     : [CH.{channel + 1}] {status}");
            if (status != XLDefine.XL_Status.XL_SUCCESS)
            {
                CanXL.PrintFunctionError();
                return Task.FromResult(status);
            ...
            GSystem.Logger.Info ($"Set Notification      : [CH.{channel + 1}] {status}, _eventHandle: {_eventHandle[channel]}");
            GSystem.TraceMessage($"Set Notification      : [CH.{channel + 1}] {status}, _eventHandle: {_eventHandle[channel]}");
            if (status != XLDefine.XL_Status.XL_SUCCESS)
            {
                CanXL.PrintFunctionError();
                return Task.FromResult(status);
            ...
            status = CanXL.Driver.XL_ActivateChannel(_portHandle[channel], CanXL._accessMask[channel], XLDefine.XL_BusTypes.XL_BUS_TYPE_CAN, XLDefine.XL_AC_Flags.XL_ACTIVATE_RESET_CLOCK);
            ...
            status = CanXL.Driver.XL_GetDriverConfig(ref CanXL._driverConfig);
            if (status != XLDefine.XL_Status.XL_SUCCESS)
            {
                CanXL.PrintFunctionError();
                return Task.FromResult(status);
            ...
            if (CanXL._canFdModeNoIso > 0)
            {
                GSystem.Logger.Info ($"CAN FD mode           : [CH.{channel + 1}] NO ISO");
                GSystem.TraceMessage($"CAN FD mode           : [CH.{channel + 1}] NO ISO");
            }
            else
            {
                GSystem.Logger.Info ($"CAN FD mode           : [CH.{channel + 1}] ISO");
                GSystem.TraceMessage($"CAN FD mode           : [CH.{channel + 1}] ISO");
            }

            GSystem.Logger.Info ($"TX Arb. BitRate       : [CH.{channel + 1}] " + CanXL._driverConfig.channel[CanXL._channelIndex[channel]].busParams.dataCanFd.arbitrationBitRate
                            + $"Bd, Data Bitrate: " + CanXL._driverConfig.channel[CanXL._channelIndex[channel]].busParams.dataCanFd.dataBitRate + "Bd");
            GSystem.TraceMessage($"TX Arb. BitRate       : [CH.{channel + 1}] " + CanXL._driverConfig.channel[CanXL._channelIndex[channel]].busParams.dataCanFd.arbitrationBitRate
                            + $"Bd, Data Bitrate: " + CanXL._driverConfig.channel[CanXL._channelIndex[channel]].busParams.dataCanFd.dataBitRate + "Bd");
        ...
        public Task<XL_Status> ClosePort(int channel)
        {
            ...
            TraceMessage($"ClosePort - NFCTouchFD");
        ...
        public StringBuilder GetEventString(XLcanRxEvent xlEvent)
        {
            int dlcLength = GetLengthDLC(xlEvent.tagData.canRxOkMsg.dlc);
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.Append($"{GetCanID(xlEvent.tagData.canRxOkMsg.canId),4:X}h  {dlcLength,2}  ");
            for (int i = 0; i < dlcLength; i++)
                stringBuilder.Append($"{xlEvent.tagData.canRxOkMsg.data[i]:X02} ");
            return stringBuilder;
        }
        ...
        public XL_Status WriteFrameStd(int channel, UInt32 id, XL_CANFD_DLC dlc, byte[] data, bool logging = false, string remarks = "")
        {
            ...
            xlEventCollection.xlCANFDEvent[0].tagData.canId = (uint)id;
        ...
        public XL_Status Send_NM(int channel, bool logging = false)
        {
            ...
            XLDefine.XL_CANFD_DLC dlc = XLDefine.XL_CANFD_DLC.DLC_CANFD_32_BYTES;
            byte[] data = new byte[32]
            {
                0x80, 0x50, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00,
        ...
        public XL_Status Send_DefaultSession(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: DEFAULT_SESSION");
        }
        ...
        public XL_Status Send_ExtendedSession(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: EXTENDED_SESSION");
        }
        ...
        public XL_Status Send_Bootloader(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: BOOTLOADER");
        }
        ...
        public XL_Status Send_RequestSeedkey(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: REQUEST_SEEDKEY");
        }
        ...
        public XL_Status Send_RequestSeedkeyFlow(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging);
        }
        ...
        public XL_Status Send_GeneratedSeedkey(int channel, byte[] seedkey, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: SEND_GENERATED_SEEDKEY");
        }
        ...
        public XL_Status Send_SerialNumberWrite(int channel, byte[] serialNumber, bool logging = false)
        {
            ...
            byte[] data = new byte[8]
            {
                0x10, 0x17, 0x2E, 0xF1, 0x8C, serialNumber[0], serialNumber[1], serialNumber[2]
            };
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: SERIAL_NUMBER_WRITE");
        }
        ...
        public XL_Status Send_SerialNumberRead(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: SERIAL_NUMBER_READ");
        }
        ...
        public XL_Status Send_ManufactureWrite(int channel, byte[] manufacture, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: MANUFACTURE_WRITE");
        }
        ...
        public XL_Status Send_ManufactureRead(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: MANUFACTURE_READ");
        }
        ...
        public XL_Status Send_PLight(int channel, bool onOff, bool logging = false)
        {
            ...
            byte[] data = new byte[32]
            {
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, lamp,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
            };
            string state = (onOff) ? "ON" : "OFF";
            return WriteFrameStd(channel, id, dlc, data, logging, $"CMD: P-LIGHT_{state}");
        }
        ...
        public XL_Status Send_DTC_Erase(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: DTC_ERASE");
        }
        ...
        public XL_Status Send_HWVersion(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: HW_VERSION_READ");
        }
        ...
        public XL_Status Send_SWVersion(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: SW_VERSION_READ");
        }
        ...
        public XL_Status Send_PartNumber(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: PART_NUMBER_READ");
        }
        ...
        public XL_Status Send_RXSWin(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: RXSWIN");
        }
        ...
        public XL_Status Send_SupplierCodeRead(int channel, bool logging = false)
        {
            ...
            return WriteFrameStd(channel, id, dlc, data, logging, "CMD: SUPPLIER_CODE_READ");
        }
        ...
        public XL_Status Send_XCPConnect(int channel, bool logging = false)
        {
            ...
            XLDefine.XL_CANFD_DLC dlc = XLDefine.XL_CANFD_DLC.DLC_CAN_CANFD_2_BYTES;
            byte[] data = new byte[2]
            {
                0xFF, 0x00
            };
            XL_Status statusResult = WriteFrameStd(channel, id, dlc, data, logging, "CMD: XCP_CONNECT");
            return statusResult;
        }
        ...
        public XL_Status Send_XCPDisconnect(int channel, bool logging = false)
        {
            ...
            XL_Status statusResult = WriteFrameStd(channel, id, dlc, data, logging, "CMD: XCP_DICONNECT");
            return statusResult;
        }
        ...
        public XL_Status Send_Upload(int channel, byte count, bool logging = false)
        {
            ...
            XL_Status statusResult = WriteFrameStd(channel, id, dlc, data, logging, $"CMD: UPLOAD size={count}");
            return statusResult;
        }
        ...
        public XL_Status Send_ShortUpload(int channel, uint address, byte count, bool logging = false, string remarks = "")
        {
            ...
            if (logging)
            {
                string logString = $"000000 CH.{channel + 1} Tx {GetEventString(xlEventCollection.xlCANFDEvent[0])}  SHORT_UPLOAD size={count} addr=0x{address:X08} {remarks}";
                GSystem.TraceMessage(logString);
            }
            return statusResult;
        }
        ...
        public XL_Status Send_SetMTA(int channel, uint address, bool logging = false, string remarks = "")
        {
            ...
            xlEventCollection.xlCANFDEvent[0].tagData.dlc = dlc;
            xlEventCollection.xlCANFDEvent[0].tagData.msgFlags = XL_CANFD_TX_MessageFlags.XL_CAN_TXMSG_FLAG_BRS | XL_CANFD_TX_MessageFlags.XL_CAN_TXMSG_FLAG_EDL;
            xlEventCollection.xlCANFDEvent[0].tagData.data[0] = 0xF6;
            xlEventCollection.xlCANFDEvent[0].tagData.data[1] = 0x00;
            xlEventCollection.xlCANFDEvent[0].tagData.data[2] = 0x00;
            xlEventCollection.xlCANFDEvent[0].tagData.data[3] = 0x00;
            xlEventCollection.xlCANFDEvent[0].tagData.data[4] = byteArray[0];
            xlEventCollection.xlCANFDEvent[0].tagData.data[5] = byteArray[1];
            ...
            //if (logging)
            //{
            //    string logString = $"{_tickXcpElapse[channel].GetTotalSeconds():F6} CH.{channel + 1} Tx {GetEventString(xlEventCollection.xlCANFDEvent[0])}  SET_MTA addr=0x{address:X08} {remarks}";
            //    GSystem.CapaData[channel].Info(logString.ToString());
            //}
            return statusResult;
        }
        ...
        public XL_Status Send_Security(int channel, uint address, bool logging = false)
        {
            ...
            byte[] data = new byte[8]
            {
                0xF6, 0x00, 0x00, 0x00, byteArray[0], byteArray[1], byteArray[2], byteArray[3]
            };
            XL_Status statusResult = WriteFrameStd(channel, id, dlc, data, logging, $"CMD: SECURITY addr=0x{address:X08}");
            return statusResult;
        }
        ...
        //                    uint rxCanID = receivedEvent.tagData.canRxOkMsg.canId & 0x7FFFFFFF;
        //                    if (rxCanID > 0x100)
        //                    {
        ...
        public void CancelTest(int channel, bool cancel)
        {
            ...
            if (cancel && GetTestStep(channel) > NFCTouchTestStep.Prepare)
                SetTestStep(channel, NFCTouchTestStep.CancelStart);
        }
        ...
        // -----------------------------------------------------------------------------------------------
        private bool ReceiveCan(int channel, ref XLcanRxEvent receivedEvent)
        {
            //WaitResults waitResult = (WaitResults)WaitForSingleObject(_eventHandle[channel], 300);
            //if (waitResult != WaitResults.WAIT_TIMEOUT)
            {
                if (CanXL.Driver.XL_CanReceive(_portHandle[channel], ref receivedEvent) == XL_Status.XL_SUCCESS)
                {
                    if ((receivedEvent.tagData.canRxOkMsg.canId & 0x7FFFFFFF) > 100)
                        return true;
                }
            }
            return false;
